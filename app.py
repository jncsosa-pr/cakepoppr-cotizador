...
st.markdown(
    '<h3 style="color: #4A2E2B; font-size: 18px;">1. Detalles del Cliente y Entrega</h3>',
    unsafe_allow_html=True
)
...
with c_info2:
    fecha_recogido = st.date_input(
        "Fecha de Recogido",
        datetime.strptime("2026-11-25", "%Y-%m-%d").date()
    )
...
# Sección 2: Configuración de Opciones
st.markdown(
    '<h3 style="color: #4A2E2B; font-size: 18px;">2. Configuración de Opciones (Hasta 4 alternativas)</h3>',
    unsafe_allow_html=True
)
        ...
        if incluir:
            ...
            with col_sz:
                sz = st.selectbox(
                    "Tamaño",
                    lista_tamanos,
                    index=lista_tamanos.index(def_sz),
                    key=f"sz_{i}"
                )
            ...
            with col_fn:
                fn = st.selectbox(
                    "Acabado",
                    lista_acabados,
                    index=lista_acabados.index(def_fn),
                    key=f"fn_{i}"
                )
...
if opciones_validas:
    ...
    st.markdown(
        '<h3 style="color: #4A2E2B; font-size: 18px;">3. Resumen Financiero Automático</h3>',
        unsafe_allow_html=True
    )
...
"""
    ...
    st.markdown(
        '<h3 style="color: #4A2E2B; font-size: 18px;">4. Mensaje Completo para enviar al Cliente</h3>',
        unsafe_allow_html=True
    )

    st.text_area(
        "Haz clic en cualquier parte del cuadro, selecciona todo (Ctrl+A / Cmd+A), copia y pega en WhatsApp:",
        texto_whatsapp,
        height=380
    )
